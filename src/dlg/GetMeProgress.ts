 ...
 * 
 * - locked    → null (module not started or not yet unlocked)
 * - available → "grammar" (first step: read the grammar intro)
 * - in_progress → "practice" (grammar done, practice underway)
    ...
    async do(req: GetMeProgressRequest, userContext?: UserContext): Promise<GetMeProgressResponse> {
        ...
        const modules: ModuleProgressEntry[] = viewedModules.map(m => {
            ...
            return {
                ...
                testUnlocksAt: null,   // requires F10 practice session completedAt — not available yet
