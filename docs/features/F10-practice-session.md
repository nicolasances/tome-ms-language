...
| Covered at rung *r* | A practice item has been served a tier-*r* exercise in a completed session **and answered correctly** — on the first attempt, on a retry, or via F13 verification. Correctness is checked server-side against the session's answer log, not assumed from the retry loop |
...
- **On session completion:**
  ...
  - **Track rung coverage**: record every practice item that was served a current-rung exercise this session **and answered correctly** into that rung's `itemIds` via `UserModuleProgressStore.appendRungCoverage` (set-union, de-duplicated). Vocabulary items and grammar concepts go into the same array. An item served an exercise it never got right is simply not credited — it stays uncovered and resurfaces in the next session's unseen reservation. This is not an error and never blocks completing the session.
...
- **Correctness is verified server-side, not assumed.** The retry queue is what *drives* a miss back around until it is right, but that loop is executed by the client, and `retryQueue` cannot evidence it: `SubmitPracticeAnswer` pushes on every wrong answer and nothing removes on a correct retry, so after one miss the queue stays non-empty for the rest of the session regardless of what the user does. Completion therefore derives correctness from the session's own answer log — an exercise counts as correct when it has at least one `isCorrect` answer, or when it appears in `verifiedExerciseIds` (F13 accepts an answer without flipping `isCorrect` on a practice session). So when a rung phase completes, every item really has been answered correctly at that rung — some on the first attempt, some after the answer was revealed.
- There is **no per-item rung state and no earned advancement**, so nothing can get stuck: an uncredited item just returns in the next session.
...
- **OQ-04** — appearance alone is not enough; a **correct** answer is required. The session collects the linked item ids of the current-rung exercises that have at least one correct answer (primary pass, retry queue, or F13 verification) and passes them to `appendRungCoverage`. Crediting on mere appearance was the original reading, but the retry loop that would have made the two equivalent is client-driven and unverifiable server-side.
