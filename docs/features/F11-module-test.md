...
The selection applies a **60% hard floor for `translation_active` exercises** (`MODULE_TEST_MIN_TRANSLATION_ACTIVE_PERCENT`) using a split-selection pattern: F08 is called first on the `translation_active`-only pool to fill that share, then on the remaining pool for the rest. If the `translation_active` pool is smaller than the floor, all available exercises of that type are used (graceful cap — no hard failure).
...
- `POST /users/:userId/moduleTests/:attemptId/submit` — finalize the attempt; computes `score` from the final per-exercise correctness, sets `passed` and `takenAt`, updates mastery (F06), records the attempt in UserModuleProgress test history (F07), and on pass transitions the module to `completed` and freezes its User Proficiency Score (F07).
...
- Submitting (`POST …/submit`): compute the score as the percentage of exercises whose final `isCorrect` is true (unanswered exercises count as wrong). Determine pass/fail against `testPassThreshold`.
  ...
  - On pass: compute the module's **User Proficiency Score** and store it on UserModuleProgress ([F07](./F07-user-module-progress.md)). The test component reads the user's **first submitted** attempt — via `ModuleTestAttemptStore.findFirstSubmittedByUserAndModule` — and needs its per-answer `isCorrect` values, not the rolled-up `score` on `UserModuleProgress.testAttempts`. This relies on the guarantee below that all submitted attempts are persisted regardless of outcome, and on the test being single-pass with no retry queue. Nothing about the grading or the pass gate changes: the UPS is a read-side derivation over data this feature already produces.
...
- **Constraint** — No fresh-vs-repeat split and no coverage override; the test draws from the same pool as practice via F08 selection, with a 60% hard floor for `translation_active` exercises applied through split-selection (`MODULE_TEST_MIN_TRANSLATION_ACTIVE_PERCENT`).
