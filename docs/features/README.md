...
- **No live AI in sessions.** The only AI touchpoints in this microservice are explicit, on-demand user actions: *explain my mistake* (F12), *translation answer verification* (F13).
- **AI is always CEFR-aware.** Every AI call (F12–F13) must receive the user's CEFR level so output is pitched correctly.
...
- The AI client used by F12–F13 is treated as an external capability; every AI-dependent feature must be testable with the client mocked.
