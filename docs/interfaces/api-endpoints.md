...
**Used for:** Starting a new Module Test attempt for a user and module (Step 3). Verifies all eligibility conditions (see above). Draws exactly 20 exercises via F08's mastery-aware selection — unconstrained (no fresh/repeat split, no coverage override). Returns the 20 questions **without** correct answers so the client can render immediately. For `multiple_choice` exercises the response includes a `choices` array (the correct answer plus its distractors, sorted alphabetically so the set is stable and the answer's position is not revealed); `answer`/`alternativeAnswers`/`userContributedAnswers`/`distractors` are not returned. Returns **409** with `{ code: 409, message: "...", attemptId }` if an active (un-submitted) attempt already exists — the client resumes it via `GET .../moduleTests/:attemptId`. Returns **400** if the module is already `completed` (no retakes).
...
**Used for:** Resuming an in-progress module test after the app is closed. Returns the full attempt state — `exerciseIds`, `answers` recorded so far, `currentPosition` — plus the exercise objects **without** correct answers, so the client restores the in-progress UI. Exercises are returned in the stored `exerciseIds` order (so `currentPosition` lines up), and `multiple_choice` exercises carry the same deterministic `choices` array as on start. An in-progress attempt is always resumable regardless of unlock timing (it started legitimately).
