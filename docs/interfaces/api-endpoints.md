...
| POST | `/users/:userId/modules/:moduleId/rePractice` | Reset a completed module for a new pass (F25) |

### GET /me/progress
**Used for:** The single BFF-style aggregate read that powers the Home dashboard and the Module map: the user's CEFR rollup across all six tiers (`levels`) plus the per-module status/step/progress list for the viewed level (`?cefrLevel`, defaults to the user's current level). For the in-progress module it also surfaces `testUnlocksAt` / `testRetryAvailableAt` (sourced from F11) so the app can render a local unlock countdown without a second request. Each module entry also includes `vocabularyItemsPracticedCount` — the number of unique vocabulary items the user has encountered across all practice sessions for that module — used by the Module Overview screen to render the Step 2 coverage progress bar. It also exposes the practice ladder's (F10) per-rung progress: `currentRung` (1–3, the rung the module is currently practising at), `currentRungCoverage` (`coveredCount`/`totalCount` of the module's combined vocabulary + grammar-concept items covered at `currentRung`), and `fullyCompletedRungs` (rungs fully behind `currentRung`; `3` once the whole ladder — or a pre-ladder-completed module — is done) — so the Module Overview screen can render a rung-aware coverage bar instead of the single module-wide count. Finally, every **completed** module carries `proficiency` — its User Proficiency Score (UPS): `{ score, testScore, practiceScore, basis, passNumber }`, all 0–100 except `basis` and `passNumber`, answering "how hard was this module" where `completionPct` only answers "did I finish it". `passNumber` (F25) says which pass through the module produced the score, so the client can tell an original score from a re-practised one. It is `null` for any module that is not `completed`, and also for a completed module holding no submitted test attempt to score. `basis` (`full` | `practice-rung2-only` | `practice-rung3-only` | `test-only`) says which inputs the score could be computed from, so a `test-only` score is never mistaken for a flawless practice run; `practiceScore` is `null` exactly when `basis` is `test-only`. The score reports the **most recently completed pass** (F25) — a re-practice can move it up or down. It is computed once per pass and stored — a completed module carrying no score, or one computed by an older formula version, is recomputed against the record's current pass and stored on read, so the first call after a deploy is slow and every later call is a plain read. The cross-level "weakest modules" overview is assembled client-side by merging the per-level responses; there is no dedicated endpoint. Replaces the former `GET /me/moduleProgress[?cefrLevel][/:moduleId]`.
**Request & Response:** `GetMeProgressRequest` / `GetMeProgressResponse` in `src/dlg/user/GetMeProgress.ts`

### POST /users/:userId/modules/:moduleId/rePractice
**Used for:** Letting a user who has already completed a module start it over from scratch — grammar, the full practice ladder, the test — replacing its User Proficiency Score with the new pass's result (F25). Resets the module's progress record: `status` → `available`, `currentRung` → `1`, `rungCoverage` → `[]`, `startedAt`/`completedAt`/`practiceCompletedAt` → `null`, `passNumber` → previous + 1. `testAttempts` and `proficiency` are preserved — the score stays invisible (only a `completed` module's proficiency is ever surfaced) until the new pass completes and replaces it. The reset is **irreversible**: there is no un-reset and no abandon. Refuses with `404` when no progress record exists for the user + module, `400` when the module's status is not `completed`, and `409` — carrying `sessionId` or `attemptId` — when an open practice session or test attempt would otherwise be stranded by the reset; the client resumes it (same shape `StartPracticeSession` / `StartModuleTest` already return on their own 409s) and retries.
**Request & Response:** `RePracticeModuleRequest` / `RePracticeModuleResponse` in `src/dlg/moduleProgress/RePracticeModule.ts`
...
| POST | `/users/:userId/practiceSessions/:sessionId/complete` | Mark the session complete; updates mastery and advances the module along the practice ladder |

### POST /users/:userId/modules/:moduleId/practiceSessions
**Used for:** Starting Step 2 (practice) for a user and module. Filters the module's exercise pool to the rung the module is currently practising at (F10's practice ladder — 1 recognition, 2 cued production, 3 free production), then selects `practiceSessionSize` exercises via mastery-aware selection (F08) with the coverage override applied — at least `PRACTICE_MIN_UNSEEN_VOCAB_PERCENT`% of exercises must target practice items (vocabulary items **or** grammar concepts) not yet covered at that rung. Transitions `UserModuleProgress` to `in_progress`. Returns the session id, the full exercise objects, and `currentRung`. Rejects with 409 if an active session already exists for this user+module, and with 400 if the module's bank holds no exercise at the current rung.
...
**Used for:** Completing a practice session. Rejects with **400** when any exercise in the session has not been answered correctly yet — the missed-retry loop must be driven to the end first; the error carries `outstandingExerciseIds`, and the refusal is atomic (nothing is written). An exercise counts as correct when it has at least one correct answer or was accepted by F13 verification. Otherwise: updates mastery scores via the SRS algorithm (F06) for every exercise attempted. Records the session's practice items — vocabulary items and grammar concepts alike — as covered at the module's current rung in `UserModuleProgress.rungCoverage` (F07). If that covers every practice item in the module, the rung phase completes: its `completedAt` is stamped and `currentRung` advances. If the rung just completed was the last one, sets `practiceCompletedAt` (starting the `testUnlockDelayHours` countdown). Returns the rung state before and after the session — `currentRung`, `rungCompleted`, `ladderCompleted`, `rungsCompletedBefore`/`After`, `rungCoverageBefore`/`After`, and module-wide `vocabularyCoverage` — so the app can animate the recap's two concentric rings and decide whether to offer another practice session or route toward the Module Test. `step2Complete` and `unseenVocabCount` are kept as aliases for clients predating the ladder.
...
**Used for:** Starting a new Module Test attempt for a user and module (Step 3). Verifies all eligibility conditions (see above). Draws exactly 20 exercises via F08's mastery-aware selection — unconstrained (no fresh/repeat split, no coverage override). Returns the 20 questions as **full exercise objects — the same payload and shape as a practice session** (including `answer` and `distractors`), so the frontend can reuse the same exercise components. Returns **409** with `{ code: 409, message: "...", attemptId }` if an active (un-submitted) attempt already exists — the client resumes it via `GET .../moduleTests/:attemptId`. Returns **400** if the module is already `completed` (no retakes).
...
**Used for:** Resuming an in-progress module test after the app is closed. Returns the full attempt state — `exerciseIds`, `answers` recorded so far, `currentPosition` — plus the full exercise objects (same payload/shape as a practice session) so the client restores the in-progress UI. Exercises are returned in the stored `exerciseIds` order, so `currentPosition` lines up with the rendered list. An in-progress attempt is always resumable regardless of unlock timing (it started legitimately).
